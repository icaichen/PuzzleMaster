...
// ─── 题型 1: 等式填空 ─────────────────────────────────────────
const equationFill = (rng, difficulty, seed) => {
    // 根据难度决定数字范围和运算符
...
// ─── 题型 2: 数列推理 ─────────────────────────────────────────
const sequencePuzzle = (rng, difficulty, seed) => {
...
// ─── 题型 3: 魔方阵 ───────────────────────────────────────────
const magicSquarePuzzle = (rng, difficulty, seed) => {
    ...
    if (n === 3) {
        ...
        square = Array.from({ length: n }, (_, i) => Array.from({ length: n }, (_, j) => i * n + j + 1));
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n; j++) {
...
// ─── 题型 4: 逻辑推理（谁在说谎）─────────────────────────────
const liarPuzzle = (rng, difficulty, seed) => {
...
export class TemplateEngine {
    ...
    generate(difficulty, seed) {
        ...
        // 筛选适合当前难度的题型
        const available = GENERATORS.filter(g => difficulty >= g.minDiff && difficulty <= g.maxDiff);
        ...
        const story = storyGen.generate(PuzzleType.MATH, result.title, undefined, result.question);
        return {
