 ...
 * 最短解步数是否落在难度对应区间内。
 */
import { PuzzleType, Direction, } from '../models/PuzzleData';
...
// ─── 主引擎类 ─────────────────────────────────────────────────
export class SlidingBlockEngine {
    constructor() {
        this.type = PuzzleType.SLIDING_BLOCK;
        this.isBank = false;
    ...
    generate(difficulty, seed) {
        ...
        for (let attempt = 0; attempt < MAX_GENERATION_ATTEMPTS; attempt++) {
            ...
            const story = storyGen.generate(PuzzleType.SLIDING_BLOCK, this.getDifficultyTitle(difficulty), undefined, `滑动方块为目标腾出道路。将目标块（2×2）移动到出口即为通关。此局最优解需要 ${solution.length} 步。`);
            ...
            const puzzleData = {
                ...
                hints,
                seed: actualSeed,
                visual_elements: visualElements, // VisualElement[]
            };
            return puzzleData;
        }
    ...
    buildGoalState() {
        ...
        for (let y = 0; y < BOARD_HEIGHT; y++) {
            grid[y] = new Array(BOARD_WIDTH).fill(false);
        }
        const placedBlocks = [];
        // 首先放置目标方块（曹操）在出口位置
     ...
     * @returns 是否成功放置所有方块
     */
    placeBlocksBacktrack(grid, defs, index, placed) {
        ...
        for (const pos of positions) {
            // 放置
            const block = { ...def, x: pos.x, y: pos.y };
     ...
     * 检查是否可以在指定位置放置方块。
     */
    canPlaceAt(grid, x, y, width, height) {
     ...
     * 在占用矩阵中标记或清除方块位置。
     */
    markGrid(grid, x, y, width, height, value) {
        for (let dy = 0; dy < height; dy++) {
            for (let dx = 0; dx < width; dx++) {
    ...
    reverseWalk(goalModel, steps) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            const filteredMoves = allMoves.filter(move => {
                if (lastBlockId === null || lastDirection === null)
                    return true;
                return !(move.blockId === lastBlockId &&
     ...
     */
    bfsSolve(initialModel) {
        const startModel = initialModel.clone();
        if (startModel.checkWin())
            return [];
        const startKey = startModel.serialize();
        const visited = new Set([startKey]);
        const queue = [{ model: startModel, path: [] }];
     ...
     * @returns 三条中文提示字符串
     */
    generateHints(solution, initialModel) {
        // 提示 1：第一步描述
...
//# sourceMappingURL=SlidingBlockEngine.js.map