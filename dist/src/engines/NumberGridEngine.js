...
export class NumberGridEngine {
    constructor() {
        this.type = PuzzleType.NUMBER_GRID;
        this.isBank = false;
    ...
    generate(difficulty, seed) {
        ...
        const story = storyGen.generate(PuzzleType.NUMBER_GRID, '不等号数独', undefined, `在 ${size}×${size} 的网格中填入数字 ${digits}，使每行每列不重复，并满足所有 > < 约束。`);
        return {
    ...
    // ─── Clue removal ───────────────────────────────────────
    removeCells(solution, size, difficulty) {
        ...
        const clues = Array.from({ length: size }, () => Array(size).fill(0));
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
    ...
    generateConstraints(solution, size, difficulty) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (v1 === v2)
                continue; // shouldn't happen in a Latin square
...
//# sourceMappingURL=NumberGridEngine.js.map