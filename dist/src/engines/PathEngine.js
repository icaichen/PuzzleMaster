...
export class PathEngine {
    constructor() {
        this.type = PuzzleType.PATH_FINDING;
        this.isBank = false;
    ...
    generate(difficulty, seed) {
        ...
        const story = storyGen.generate(PuzzleType.PATH_FINDING, '一笔画路径', undefined, `在 ${gridSize}×${gridSize} 的点阵中画一条连续线，访问所有点各一次。起点(S)和终点(E)已标记。`);
        return {
    ...
    buildGridAdjacency(n) {
        ...
        for (let r = 0; r < n; r++) {
            for (let c = 0; c < n; c++) {
                const idx = r * n + c;
                if (c + 1 < n) {
                    adj[idx].push(r * n + (c + 1));
                    adj[r * n + (c + 1)].push(idx);
                }
                if (r + 1 < n) {
                    adj[idx].push((r + 1) * n + c);
                    adj[(r + 1) * n + c].push(idx);
                }
            }
        }
    ...
    dfsHamiltonian(current, total, adjacency, visited = Array(total).fill(false), path = []) {
    ...
    selectGivenEdges(solutionPath, ratio) {
        const edges = [];
    ...
    // ─── Hints ──────────────────────────────────────────────
    generateHints(difficulty, gridSize) {
