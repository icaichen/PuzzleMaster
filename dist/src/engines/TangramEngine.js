...
export class TangramEngine {
    ...
    generate(difficulty, seed) {
        ...
        const pool = SHAPES.filter(s => {
            if (difficulty <= 2)
                return s.cells.length <= 6;
            if (difficulty <= 5)
                return s.cells.length <= 9;
            if (difficulty <= 8)
                return s.cells.length <= 14;
        ...
        const pieceCount = difficulty <= 2 ? 3
            : difficulty <= 5 ? 4
                : difficulty <= 7 ? 5
                    : 6;
        const maxCoord = Math.max(...targetCells.map(([r, c]) => Math.max(r, c)));
        ...
        if (!pieces || pieces.length < 2)
            return null;
        // Normalize pieces
        const normalized = pieces.map((cells, i) => ({
            id: i,
        ...
        const story = storyGen.generate(PuzzleType.JIGSAW, `七巧板 · ${shape.nameCN}`, shape.storySetup, question);
    ...
    // ─── 分解形状 ──────────────────────────────────────────
    decomposeShape(cells, pieceCount) {
        if (pieceCount <= 1)
            return [cells];
    ...
    normalizeToOrigin(cells) {
        const minR = Math.min(...cells.map(c => c[0]));
        const minC = Math.min(...cells.map(c => c[1]));
        return cells.map(([r, c]) => [r - minR, c - minC]);
    }
    normalizePiece(cells) {
        return this.normalizeToOrigin(cells);
    }
}
//# sourceMappingURL=TangramEngine.js.map