...
export class VisualPuzzleView {
    constructor(container) {
        this.mode = 'path_finding';
        this.pathState = null;
        this.numState = null;
        this.tangramState = null;
        this.title = '';
        this.scenario = '';
        this.hints = [];
        this.picarat = 10;
        this.puzzleType = PuzzleType.PATH_FINDING;
        this.pieceCount = 0;
        this.cellSize = 56;
        this.gridOffsetX = 0;
        this.gridOffsetY = 0;
    ...
    setup(params) {
        ...
        if (this.mode === 'path_finding') {
            ...
            this.tangramState = params.state;
            if (this.tangramState)
                this.pieceCount = this.tangramState.pieceCount;
            this.cellSize = this.tangramState && this.tangramState.gridSize <= 5 ? 48 : 38;
        }
    }
    onWin(cb) { this.onWinCb = cb; }
    onNext(cb) { this.onNextCb = cb; }
    onHint(cb) { this.onHintCb = cb; }
    render() {
        ...
        if (this.submitted)
            this.buildAnswerReveal(answerBox);
    ...
    drawIllustration(w) {
        ...
        if (words.length > maxChars) {
            ...
            let br = mid;
            while (br < words.length && words[br] !== '、' && words[br] !== '。' && words[br] !== '，' && words[br] !== ' ')
                br++;
            if (br >= words.length - 2)
                br = mid;
    ...
    drawTangramIllustration(w, H) {
        ...
        ctx.beginPath();
        ctx.arc(gx - 22, gy + 74, 6, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(gx + 22, gy + 74, 6, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ctx.arc(gx - 6, gy + 14, 3, 0, Math.PI);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(gx + 6, gy + 14, 3, 0, Math.PI);
        ctx.stroke();
        ...
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ctx.arc(gx - 6, gy + 16, 6, 0, Math.PI * 2);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(gx + 6, gy + 16, 6, 0, Math.PI * 2);
        ctx.stroke();
    ...
    drawTangram() {
        ...
        for (const pp of this.placedPieces) {
            this.drawPieceBlock(ctx, ox, oy, cs, pp.cells, this.P.tangramPlaced, this.P.tangramPlacedBorder);
        }
        // ── Ghost preview ──
        if (this.selectedPieceIdx >= 0 && this.selectedPieceIdx < pieces.length) {
            ...
            this.drawPieceBlock(ctx, ox, oy, cs, ghost, valid ? this.P.tangramGhost : this.P.tangramGhostInvalid, valid ? 'rgba(255,255,255,0.25)' : 'rgba(255,68,68,0.4)');
        }
        // ── Pieces tray ──
        ...
        for (let i = 0; i < pieces.length; i++) {
            const p = pieces[i];
            if (this.placedPieces.some(pp => pp.pieceId === p.id))
                continue;
    ...
    drawPieceBlock(ctx, ox, oy, cs, cells, fillColor, strokeColor) {
        for (const [r, c] of cells) {
            const x = ox + c * cs, y = oy + r * cs;
    ...
    checkAnswer() {
        if (this.submitted)
            return;
        if (this.mode === 'path_finding')
            this.correct = this.checkPathAnswer();
        else if (this.mode === 'number_grid')
            this.correct = this.checkGridAnswer();
        else if (this.mode === 'tangram')
            this.correct = this.checkTangramAnswer();
        this.submitted = true;
    ...
    checkTangramAnswer() {
        ...
        for (const pp of this.placedPieces) {
            for (const [r, c] of pp.cells) {
                const key = `${r},${c}`;
                if (placed.has(key) || !target.has(key))
                    return false;
                placed.add(key);
            }
        }
    ...
    handleClick(e) {
        ...
        if (this.mode === 'path_finding')
            this.handlePathClick(mx, my);
        else if (this.mode === 'number_grid')
            this.handleGridClick(mx, my);
        else if (this.mode === 'tangram')
            this.handleTangramClick(mx, my);
        this.drawCanvas();
    }
    handleTangramClick(mx, my) {
        if (!this.tangramState)
            return;
        ...
        if (gr >= 0 && gr < gridSize && gc >= 0 && gc < gridSize && my < oy + targetH) {
            if (this.selectedPieceIdx >= 0) {
                ...
                const offset = t.map(([r, c]) => [r - minR + gr, c - minC + gc]);
    ...
    handleKey(key) {
        ...
        if (this.mode === 'tangram') {
            if (key === 'r' || key === 'R')
                this.selectedPieceRotation = (this.selectedPieceRotation + 1) % 4;
            else if (key === 'f' || key === 'F')
        ...
        if (this.mode !== 'number_grid' || !this.selectedCell || !this.numState)
            return;
        ...
        if (key >= '1' && key <= String(mv)) {
            ...
            if (nr < mv && this.numState.grid[nr][nc] !== 0) {
                nc++;
                if (nc >= mv) {
                    nc = 0;
                    nr++;
...
//# sourceMappingURL=VisualPuzzleView.js.map