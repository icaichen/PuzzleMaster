...
export function createPuzzlePage(width) {
...
//  标题栏更新器
// ═══════════════════════════════════════════════════════════
export function createHeader(type, title, picarat, puzzleNum) {
...
export function createButtonRow() {
    ...
    const make = (text, bg, color = '#fffef5', border) => {
    ...
    return {
        ...
        onNext: (cb) => { nextCb = cb; },
        onHint: (cb) => { hintCb = cb; },
        setHintCoins: (n) => { hintCoins = n; hintBtn.textContent = '💡 提示 (' + n + ')'; },
    };
}
export function createHintPanel(hints, initialCoins = 3) {
    ...
    const reveal = () => {
        if (coins <= 0 || revealed >= hints.length)
            return false;
        coins--;
        const idx = revealed++;
...
//  Canvas 背景装饰 — 羊皮纸纹理
// ═══════════════════════════════════════════════════════════
export function drawParchmentBg(ctx, w, h) {
    // 暖色渐变背景
