/**
 * KlotskiModel.ts — 华容道数据模型
 *
 * 维护棋盘状态、碰撞检测占用矩阵，以及方块移动逻辑。
 * 设计为纯数据层，不包含任何渲染或输入处理逻辑。
 */

import { KlotskiBlock, KlotskiState, Direction } from './PuzzleData';

export class KlotskiModel {
  boardWidth: number;
  boardHeight: number;
  blocks: KlotskiBlock[];
  exitX: number;
  ...
  constructor(state: KlotskiState) {
  ...
  getBlockById(id: string): KlotskiBlock | undefined {
    return this.blocks.find(b => b.id === id);
  }

  getBlockAt(x: number, y: number): KlotskiBlock | null {
  ...
  canMove(blockId: string, direction: Direction): boolean {
    ...
    const { dx, dy } = KlotskiModel.directionDelta(direction);
  ...
  setState(state: KlotskiState): void {
    this.boardWidth = state.boardWidth;
    this.boardHeight = state.boardHeight;
    this.blocks = state.blocks.map(b => ({ ...b }));
    this.exitX = state.exitX;
  ...
  clone(): KlotskiModel {
    return new KlotskiModel(this.getState());
  }
