...
export class NumberGridLayout implements IPuzzleLayout {
  private wrapper!: HTMLElement;
  private canvas!: HTMLCanvasElement;
  private ctx!: CanvasRenderingContext2D;
  private state: NumberGridState | null = null;
  ...
  // Visual renderer
  private renderer = new NumberGridRenderer();
  ...
  render(params: PuzzleRenderParams): void {
    ...
    const state = params.puzzle.initial_state as NumberGridState;
    this.state = state;
  ...
  private handleClick(e: MouseEvent): void {
    ...
    if (row < 0 || row >= size || col < 0 || col >= size) {
      this.selectedCell = null; this.draw(); return;
    }
    if (grid[row][col] !== 0) {
      this.selectedCell = null; this.draw(); return;
    }
  ...
  private handleKey(key: string): void {
    if (!this.state || !this.selectedCell || this.submitted) return;
    ...
    if (key >= '1' && key <= String(mv)) {
      ...
      let nr = r, nc = c + 1;
      if (nc >= mv) { nc = 0; nr++; }
      if (nr < mv && this.state.grid[nr][nc] !== 0) { nc++; if (nc >= mv) { nc = 0; nr++; } }
  ...
  private checkAnswer(): void {
    ...
    const correct = this.playerGrid.every((row, r) =>
