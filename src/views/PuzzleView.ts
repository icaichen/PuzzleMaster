...
export class PuzzleView {
  ...
  onWin(cb: () => void): void { this.onWinCallback = cb; }
  ...
  private doRender(isNext: boolean): void {
    ...
    const checkAnswer = () => {
      ...
      const isCorrect = cleanInput.length > 0 && (
    ...
    const revealHint = () => {
      if (this.hintCoins <= 0 || this.hintsRevealed >= this.hintTexts.length) return;
  ...
  private injectStyles(): void {
    if (document.getElementById('puzzle-view-styles')) return;
