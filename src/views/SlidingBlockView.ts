...
import { Direction, SlidingBlockBlock, AssetManifest, VisualElement } from '../models/PuzzleData';
...
function getBlockLabel(block: SlidingBlockBlock): string {
  if (BLOCK_LABELS[block.id]) return BLOCK_LABELS[block.id];
  if (block.isTarget) return '目标';
  if (block.width === 2 && block.height === 1) return '横障';
  if (block.width === 1 && block.height === 1) return '方块';
  // Vertical 1×2 blocks
...
export class SlidingBlockView {
  ...
  private model: SlidingBlockModel;
   ...
   */
  setModel(model: SlidingBlockModel): void {
    this.model = model;
  ...
  render(animT?: number, winNow?: number): void {
    ...
    if (this.sceneRenderer && this.assets && this.visualElements) {
       ...
       const elements: {id: string, x: number, y: number, w: number, h: number, alpha?: number, scale?: number, glow?: boolean, isTarget?: boolean}[] = [];
   ...
   */
  private drawBlock(block: SlidingBlockBlock, px: number, py: number, winNow?: number): void {
    const ctx = this.ctx;
