...
export interface NumberGridRenderState {
...
export class NumberGridRenderer {
  private rng = seededRNG(17);

  draw(ctx: CanvasRenderingContext2D, logicalW: number, logicalH: number, rs: NumberGridRenderState): void {
    ...
    // ── 格子 ──
    const allFilled = playerGrid.every((row, r) =>
      row.every((val, c) => grid[r][c] !== 0 || val !== 0),
    );
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath(); ctx.moveTo(x + 2, y + 4); ctx.lineTo(x + cs - 2, y + 4); ctx.stroke();
        // 下边高光
        ctx.strokeStyle = 'rgba(255,255,255,0.4)';
        ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(x + 2, y + cs - 3); ctx.lineTo(x + cs - 2, y + cs - 3); ctx.stroke();
        ctx.restore();
  ...
  private drawInkArrow(
    ctx: CanvasRenderingContext2D, x: number, y: number,
    dir: number, // 0 = vertical (∧), 1 = horizontal (<)
    size: number,
  ): void {
    ctx.save();
