...
export interface TangramRenderState {
  state: TangramState;
  cellSize: number;
  gridOx: number;
  gridOy: number;
  ...
  unplacedPieces: {
    piece: PieceDef;
    trayX: number; trayY: number;
    trayW: number; trayH: number;
  }[];
  dragging: {
...
// ═══════════════════════════════════════════════════════════
//  形状轮廓路径 — 每个形状都有贝塞尔曲线定义的平滑轮廓
//  路径坐标为 [0,0]–[1,1] 归一化空间
// ═══════════════════════════════════════════════════════════

type PathCmd = ['M', number, number]
  | ['L', number, number]
  | ['Q', number, number, number, number]
  | ['C', number, number, number, number, number, number]
  | ['Z'];

const SHAPE_PATHS: Record<string, PathCmd[]> = {
  diamond: [
    ['M', 0.5, 0.05],
...
const SHAPE_THEMES: Record<string, {
  fill: string;       // 轮廓填充色
  stroke: string;     // 轮廓描边色
  accent: string;     // 装饰色 (碎片)
  bg: string;         // 背景氛围色
  pieceBase: string;  // 碎片底色
  pieceHighlight: string; // 碎片高光
}> = {
...
const SHAPE_SCENES: Record<string, string> = {
  diamond:   '✦ 珠宝行 · 展示厅 ✦',
  heart:     '♡ 情人节的信 ♡',
  tree:      '🎄 平安夜壁炉旁 🎄',
  star:      '✦ 天文台 · 星图 ✦',
...
export class TangramRenderer {
  draw(ctx: CanvasRenderingContext2D, logicalW: number, logicalH: number, rs: TangramRenderState): void {
    ...
    for (let i = 0; i < rs.unplacedPieces.length; i++) {
      if (rs.dragging && (rs.dragging as any).pieceIdx === i) continue;
  ...
  private drawTargetSilhouette(
    ctx: CanvasRenderingContext2D,
    ox: number, oy: number,
    gridSize: number, cs: number,
    targetSet: Set<string>,
    shapeKey: string,
    theme: typeof SHAPE_THEMES.diamond,
  ): void {
    ...
    if (path) {
      ...
      for (const cmd of path) {
        switch (cmd[0]) {
          case 'M': ctx.moveTo(ox + cmd[1] * totalW, oy + cmd[2] * totalH); break;
          case 'L': ctx.lineTo(ox + cmd[1] * totalW, oy + cmd[2] * totalH); break;
          case 'Q': ctx.quadraticCurveTo(
            ox + cmd[1] * totalW, oy + cmd[2] * totalH,
            ox + cmd[3] * totalW, oy + cmd[4] * totalH,
          ); break;
          case 'C': ctx.bezierCurveTo(
            ox + cmd[1] * totalW, oy + cmd[2] * totalH,
            ox + cmd[3] * totalW, oy + cmd[4] * totalH,
            ox + cmd[5] * totalW, oy + cmd[6] * totalH,
          ); break;
  ...
  private drawDragGhost(
    ctx: CanvasRenderingContext2D,
    drag: NonNullable<TangramRenderState['dragging']>,
    cs: number,
    theme: typeof SHAPE_THEMES.diamond,
  ): void {
  ...
  private getShapeKey(nameCN: string): string {
  ...
  private getBB(cells: [number, number][]): { minR: number; maxR: number; minC: number; maxC: number } {
