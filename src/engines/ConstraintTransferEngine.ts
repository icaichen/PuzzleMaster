import { PuzzleEngine, PuzzleMechanic } from '../models/PuzzleData';
import { SeededRandom } from '../models/SeededRandom';

export interface ConstraintTransferLogic {
  entities: string[];
  locations: string[]; 
  carrier: {
    capacity: number;
    requiresPilot: boolean;
    pilotEntities: string[]; // Entities that can pilot the carrier
  };
  conflicts: Array<{
    predator: string;
    prey: string;
...
export class ConstraintTransferEngine implements PuzzleEngine {
  readonly mechanic = PuzzleMechanic.CONSTRAINT_TRANSFER;
  readonly isBank = false;
  private rng: SeededRandom = new SeededRandom(0);

  generateLogic(difficulty: number, seed?: number): ConstraintTransferLogic | null {
