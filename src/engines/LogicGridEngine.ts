...
export type ClueType = 'SAME' | 'NOT_SAME' | 'BEFORE' | 'NEXT_TO' | 'NOT_NEXT_TO';
...
export class LogicGridEngine implements PuzzleEngine {
    readonly type = PuzzleType.LOGIC_GRID;
    ...
    public generate(difficulty: number, seed?: number): PuzzleData {
            ...
            for (let c2 = c1; c2 < numCats; c2++) {
                for (let p1 = 0; p1 < size; p1++) {
                    for (let p2 = 0; p2 < size; p2++) {
                        ...
                        if (c1 === c2 && p1 > p2) continue; // For same category, only do p1 < p2 to prevent symmetry
    ...
    private getPermutations(n: number): number[][] {
        const res: number[][] = [];
    ...
    private checkClue(clue: Clue, assignment: number[][]): boolean {
        let p1 = -1, p2 = -1;
        for (let p = 0; p < assignment[0].length; p++) {
            if (assignment[clue.c1Idx][p] === clue.i1Idx) p1 = p;
            if (assignment[clue.c2Idx][p] === clue.i2Idx) p2 = p;
        }
    ...
    private countSolutions(clues: Clue[], numCats: number, size: number, perms: number[][]): number {
        ...
        if (numCats === 3) {
            for (let i = 0; i < numPerms; i++) {
                for (let j = 0; j < numPerms; j++) {
                    ...
                    let valid = true;
                    for (const clue of actualClues) { if (!this.checkClue(clue, assignment)) { valid = false; break; } }
                    if (valid) { count++; if (count > 1) return count; }
                }
            }
        } else if (numCats === 4) {
             for (let i = 0; i < numPerms; i++) {
