 ...
 * 通过向大模型请求，生成雷顿式的叙事情境、视觉元素映射和整合提示，
...
export class LLMEngine {
     ...
     */
    public async enrichPuzzle(mechanismPuzzle: PuzzleData): Promise<PuzzleData> {
        // 模拟 LLM API 的网络延迟
        await new Promise(resolve => setTimeout(resolve, 300));
        return this.mockLLMResponse(mechanismPuzzle);
    }

    private mockLLMResponse(puzzle: PuzzleData): PuzzleData {
        ...
        switch (puzzle.type) {
            ...
            case PuzzleType.LOGIC_GRID: // 逻辑网格
                ...
                // 替换冰冷的机制描述
                if (enriched.description && enriched.description.includes("逻辑网格")) {
