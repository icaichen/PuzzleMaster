...
const CLASSIC_BLOCK_DEFS: Array<Omit<KlotskiBlock, 'x' | 'y'>> = [
  { id: 'caocao',      width: 2, height: 2, isTarget: true },
  { id: 'guanyu',      width: 2, height: 1, isTarget: false },
  { id: 'zhangfei',    width: 1, height: 2, isTarget: false },
  { id: 'zhaoyun',     width: 1, height: 2, isTarget: false },
  { id: 'machao',      width: 1, height: 2, isTarget: false },
  { id: 'huangzhong',  width: 1, height: 2, isTarget: false },
  { id: 'zu1',         width: 1, height: 1, isTarget: false },
  { id: 'zu2',         width: 1, height: 1, isTarget: false },
  { id: 'zu3',         width: 1, height: 1, isTarget: false },
  { id: 'zu4',         width: 1, height: 1, isTarget: false },
];
...
export class KlotskiEngine implements PuzzleEngine {
   ...
   *
   * @returns 目标状态的 KlotskiModel，或 null（放置失败）
   */
  private buildGoalState(): KlotskiModel | null {
    // 构建占用矩阵
    const grid: boolean[][] = [];
    ...
    const placedBlocks: KlotskiBlock[] = [];

    // 首先放置目标方块（曹操）在出口位置
    const targetDef = CLASSIC_BLOCK_DEFS[0];
    const targetBlock: KlotskiBlock = {
      ...targetDef,
      x: EXIT_X,
      y: EXIT_Y,
    };
  ...
  private placeBlocksBacktrack(
    grid: boolean[][],
    defs: Array<Omit<KlotskiBlock, 'x' | 'y'>>,
    index: number,
    placed: KlotskiBlock[]
  ): boolean {
    ...
    for (const pos of positions) {
      // 放置
      const block: KlotskiBlock = { ...def, x: pos.x, y: pos.y };
   ...
   * @returns 游走后的初始状态模型
   */
  private reverseWalk(goalModel: KlotskiModel, steps: number): KlotskiModel {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      const filteredMoves = allMoves.filter(move => {
        ...
        return !(
          ...
          move.direction === KlotskiModel.oppositeDirection(lastDirection)
        );
      });
  ...
  private generateHints(
    ...
    // 提示 3：通用策略
    const targetMoves = solution.filter(m => m.blockId === 'caocao').length;
    const hint3 = `整体策略：此局共需 ${solution.length} 步，其中曹操需移动 ${targetMoves} 次。优先为曹操清理下方通道。`;
